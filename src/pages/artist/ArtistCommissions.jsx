...
import { FileText, CheckCircle, Clock, Check, X, ShieldAlert } from 'lucide-react';
...
const ArtistCommissions = () => {
    ...
    const fetchCommissions = async () => {
        setLoading(true);
        try {
            // Using the global getAdminCommissions endpoint which allows ADMIN & ARTIST roles
            const data = await api.getAdminCommissions();
            setCommissions(data || []);
        } catch (error) {
            console.error('Failed to load commission briefs:', error);
            addToast('error', 'Failed to retrieve spatial commission requests.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="bg-[#0D0D0D] text-[#FAF9F6] min-h-screen pt-36 pb-24 px-8">
            <div className="max-w-6xl mx-auto space-y-12">
                ...
                <div className="border-b border-white/5 pb-8">
                    <span className="text-[10px] uppercase tracking-[0.3em] text-[#C5A880]">Studio Commissions</span>
                    <h1 className="text-3xl md:text-5xl font-display font-light text-[#FAF9F6]">Commission Briefs</h1>
                    <p className="text-xs text-gray-500 tracking-wider mt-2">Inspect, moderate, and update active spatial commission creations requested by elite clients.</p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="bg-[#090909] border border-white/5 p-16 text-center text-xs text-gray-500 tracking-wider">
                        No custom commission requests have been cataloged on the platform yet.
                    </div>
                ) : (
                                ...
                                <div className="space-y-4 max-w-3xl flex-1">
                                    <div className="flex flex-wrap items-center gap-4">
                                        ...
                                        <span className="text-[10px] text-gray-500 tracking-wider">
                                            Client: <span className="text-gray-300 font-light select-all">{c.userId}</span>
                                        </span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 border-t border-white/5 pt-4 text-xs font-light tracking-widest">
                                        ...
                                        <div>
                                            <span className="text-gray-600 block text-[9px] uppercase">Target Budget</span>
                                ...
                                <div className="flex md:flex-col justify-end gap-3 md:w-48 self-start pt-2 border-t md:border-t-0 md:border-l border-white/5 md:pl-6">
                                    {c.status === 'Pending' && (
                                        <>
                                            ...
                                            >
                                                <Check size={12} /> Accept Brief
                                            </button>
                                            <button
                                                onClick={() => handleUpdateStatus(c._id, 'Rejected')}
                                                className="w-full border border-white/10 hover:border-red-900/50 hover:bg-red-950/10 text-gray-400 hover:text-red-300 text-[10px] tracking-widest uppercase py-3 font-semibold transition-all flex items-center justify-center gap-1.5"
                                            >
                                                <X size={12} /> Decline Brief
                                            </button>
                                        </>
                                    )}
                                    ...
                                    {c.status === 'Accepted' && (
                                        <button
                                            ...
                                            className="w-full bg-[#151515] hover:bg-[#C5A880] text-gray-300 hover:text-[#0D0D0D] text-[10px] tracking-widest uppercase py-3 font-semibold border border-white/5 transition-all flex items-center justify-center gap-1.5"
                                        >
                                            <Clock size={12} /> Begin Drafting
                                        </button>
                                    )}
                                    ...
                                    {c.status === 'Rejected' && (
                                        <div className="w-full text-center text-[10px] text-red-400 uppercase tracking-widest py-3">
                                            Brief Refused
                                        </div>
                                    )}
                                </div>
